...
class TestCleanShutdownMarker:
    ...
    def test_restart_timeout_still_writes_marker(self, tmp_path, monkeypatch):
        """Timed-out drain during explicit restart should preserve continuity."""
        monkeypatch.setattr("gateway.run._hermes_home", tmp_path)
        marker = tmp_path / ".clean_shutdown"
        ...
        with patch("gateway.run.GatewayRunner._drain_active_agents", new_callable=AsyncMock, return_value=({"s1": None}, True)), \
             patch("gateway.run.GatewayRunner._finalize_shutdown_agents"), \
             patch("gateway.run.GatewayRunner._update_runtime_status"), \
             patch("gateway.run.GatewayRunner._interrupt_running_agents"), \
             patch("gateway.run.GatewayRunner._running_agent_count", return_value=1), \
             patch("gateway.status.remove_pid_file"), \
             patch("tools.process_registry.process_registry") as mock_proc_reg, \
             patch("tools.terminal_tool.cleanup_all_environments"), \
             patch("tools.browser_tool.cleanup_all_browsers"):
        ...
        assert marker.exists(), ".clean_shutdown marker should exist after timed-out restart"

    def test_shutdown_timeout_without_restart_skips_marker(self, tmp_path, monkeypatch):
        """Timed-out non-restart shutdown should still skip marker (crash-safety)."""
        monkeypatch.setattr("gateway.run._hermes_home", tmp_path)
        marker = tmp_path / ".clean_shutdown"
        ...
        with patch("gateway.run.GatewayRunner._drain_active_agents", new_callable=AsyncMock, return_value=({"s1": None}, True)), \
            ...
            import asyncio
            asyncio.get_event_loop().run_until_complete(runner.stop(restart=False))

        assert not marker.exists(), ".clean_shutdown marker should not exist after timed-out non-restart shutdown"
